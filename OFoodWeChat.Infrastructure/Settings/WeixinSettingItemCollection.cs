...
namespace OFoodWeChat.Infrastructure.Settings
{
    ...
    /// </summary>
    public class WeixinSettingItemCollection : Dictionary<string, WeixinSettingItem>
    {
        public WeixinSettingItemCollection() : base(StringComparer.OrdinalIgnoreCase)
        {
        }
        ...
        /// <returns></returns>
        new public WeixinSettingItem this[string key]
        {
            get
            {
                if (!base.ContainsKey(key))
                {
                    base[key] = new WeixinSettingItem();
                }
                return base[key];
            }
            set
