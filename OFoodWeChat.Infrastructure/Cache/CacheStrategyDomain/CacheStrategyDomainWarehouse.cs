...
namespace OFoodWeChat.Infrastructure.Cache
{
    ...
    public class CacheStrategyDomainWarehouse
    {
        ...
        public static List<Type> AutoScanDomainCacheStrategy(bool autoScanExtensionCacheStrategies = false, Func<IList<IDomainExtensionCacheStrategy>> extensionCacheStrategiesFunc = null)
        {
            ...
            if (autoScanExtensionCacheStrategies)
            {
                ...
                var types = AppDomain.CurrentDomain.GetAssemblies()
                            .SelectMany(a =>
                            {
                                ...
                                {
                                    Trace.LogTrace.SendCustomLog("UseSenparcGlobal() 自动扫描程序集异常：" + a.FullName, ex.ToString());
                ...
                if (types != null)
                {
                    foreach (var type in types)
                    {
                        ...
                        {
                            Trace.LogTrace.BaseExceptionLog(new Exceptions.BaseException(ex.Message, ex));
                        }
                    }
                }
            }
            ...
            Trace.LogTrace.SendCustomLog("自动注册扩展缓存完成", exCacheLog);
