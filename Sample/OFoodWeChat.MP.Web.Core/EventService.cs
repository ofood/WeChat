...
using OFoodWeChat.MP.Web.Core.TemplateMessage;
...
namespace OFoodWeChat.MP.Web.Core
{
    /// <summary>
    /// 全局微信事件有关的处理程序
    /// </summary>
    public class EventService
    {
        ...
        {
            WeixinTrace.SendCustomLog("进入 ConfigOnWeixinExceptionFunc() 方法", ex.Message);
            try
            {
                Task.Factory.StartNew(async () =>
                {
                    var appId = WxConfig.WeixinSetting.WeixinAppId;
                    ...
                    if (ex is ErrorJsonResultException)
                    {
                        ...
                        //if (ignoreErrorCodes.Contains(jsonEx.JsonResult.ErrorCodeValue))
                        //{
                    ...
                    if (sendTemplateMessage)    // DPBMARK MP
                    {
                        ...
                        {
                            var result = await OFoodWeChat.MP.AdvancedAPIs.TemplateApi.SendTemplateMessageAsync(appId, openId, data.TemplateId,
                              url, data);
                        }
                    }                           // DPBMARK_END
                });
